...
export class CollisionsManager {
  ...
  constructor(public room: any = false) {
    if (room) {
  ...
  assignBeginCollisions(evt) {
    let bodyA: BodyWithState             = evt.bodyA,
        bodyB: BodyWithState             = evt.bodyB,
        currentPlayerBody: BodyWithState = null,
        otherBody: BodyWithState         = null;
    // cases:
    // - player hit a player
  ...
  playerHitChangePoint(playerBody, changePoint) {
    ...
    if (playerBody.isChangingScene === false) {
      ...
      this.room.nextSceneInitialPosition(contactClient, changeData).catch((err) => {
        console.error('nextSceneInitialPosition error: ' + err);
      });
    }
  }
